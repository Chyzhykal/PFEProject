...
CREATE TABLE IF NOT EXISTS `db_teacherDay`.`t_event` (
    ...
    `fkUser` INT NOT NULL,
    PRIMARY KEY (`idEvent` , `fkDay` , `fkUser`),
    UNIQUE INDEX `idEvent_UNIQUE` (`idEvent` ASC),
    INDEX `fk_t_event_t_day1_idx` (`fkDay` ASC),
    INDEX `fk_t_event_t_event1_idx` (`fkLinkedEvent` ASC),
    INDEX `fk_t_event_t_user1_idx` (`fkUser` ASC),
    CONSTRAINT `fk_t_event_t_day1` FOREIGN KEY (`fkDay`)
        REFERENCES `db_teacherDay`.`t_day` (`idDay`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_t_event_t_event1` FOREIGN KEY (`fkLinkedEvent`)
        REFERENCES `db_teacherDay`.`t_event` (`idEvent`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_t_event_t_user1` FOREIGN KEY (`fkUser`)
...
CREATE TABLE IF NOT EXISTS `db_teacherDay`.`t_event_has_t_participant` (
    ...
    CONSTRAINT `fk_t_event_has_t_participant_t_event` FOREIGN KEY (`fkEvent`)
        REFERENCES `db_teacherDay`.`t_event` (`idEvent`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_t_event_has_t_participant_t_participant1` FOREIGN KEY (`fkParticipant`)
