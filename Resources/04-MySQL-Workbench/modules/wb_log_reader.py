...
class BaseLogFileReader(object):
    ...
    def __init__(self, ctrl_be, log_file, pat, chunk_size, truncate_long_lines, append_gaps=True):
        ...
        if use_sudo:
            log_info(_this_file, "Will use sudo and dd to get contents of log file %s\n" % self.log_file_name)
            ...
            try:
                ...
                log_error(_this_file, "Invalid password to sudo %s\n" % error)
                ctrl_be.password_handler.reset_password_for('file')
                raise
        elif use_sftp:
            log_info(_this_file, "Will use sftp to get contents of log file %s\n" % self.log_file_name)
            self.log_file = SFTPInputFile(self.ctrl_be, self.log_file_name)
            self.file_size = self.log_file.size
        else:
            log_info(_this_file, "Will use plain file access to get contents of log file %s\n" % self.log_file_name)
