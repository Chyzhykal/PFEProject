...
import grt
import mforms
from workbench.db_utils import escape_sql_identifier, escape_sql_string as esc_str
...
def do_drop_object(editor, selection):
    ...
    if res == mforms.ResultOk:
        if not review_sql_code("\n".join([stmt+";" for c, stmt in statements])):
            return
...
class DependencyAnalyzer:
    ...
    def load_data(self, connection):
        ...
        grt.log_info("sqlide_grt", "Reverse engineering schema %s..." % self.schema_name)
